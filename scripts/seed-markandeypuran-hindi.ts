/**
 * Seed Hindi for Markandeya Purana chapters 4–90 from Gita Press cache.
 *
 * Chapters 1–3 keep curated highlights unless `--force`.
 *
 * Prerequisite: python scripts/extract-markandeypuran-hindi.py
 * Run: npm run seed:markandeypuran-hindi
 */
...
const PUB_PATH = resolve(ROOT, "public/data/scriptures-full/markandeypuran.json");
const CACHE_PATH = resolve(ROOT, "scripts/cache/markandeypuran-hindi.json");
...
async function main(): Promise<void> {
  log(`Seeding Markandeya Purana Hindi from Gita Press cache${FORCE ? " [force]" : ""}…`);
