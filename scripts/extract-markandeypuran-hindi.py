"""Extract Hindi for Markandeya Purana from Gita Press OCR.

Source: archive.org 7.-markandeya-purana-gita-press
  7. Markandeya Purana - Gita Press_djvu.txt

Continuous Hindi from adhyāya 4 through chapter 90.
Chapters 1–3 keep curated highlights.

Output: scripts/cache/markandeypuran-hindi.json

Run: python scripts/extract-markandeypuran-hindi.py
"""
...
IDENT = "7.-markandeya-purana-gita-press"
DJVU_FNAME = "7. Markandeya Purana - Gita Press_djvu.txt"
CACHE_PATH = ROOT / "scripts/cache/markandeypuran-hindi-djvu.txt"
JSON_PATH = ROOT / "public/data/scriptures-full/markandeypuran.json"
OUT_PATH = ROOT / "scripts/cache/markandeypuran-hindi.json"
...
HINDI_MARKERS = re.compile(
    ...
    r"नहीं|करना|हुआ|हुए|अर्थात्|क्योंकि|इसलिये|भगवान|विष्णु|पुराण|मार्कण्डेय|शिव|"
    r"धर्म|मुनि|देव|राजा|ब्रह्म|श्रवण|पाप|मोक्ष|ऋषि|यज्ञ|लोक|नारायण|इन्द्र|"
    r"जैमिनि|देवी|दुर्गा|पक्षि|विन्ध्य|द्रोण|काली|महिष)"
)
...
PAGE_HEADER = re.compile(
    r"^\*?\s*श्रीमार्कण्डेयपुराण|^\*?\s*मार्कण्डेयपुराण|^\d{1,4}\s+\*?$|"
    r"^\s*॥ [^०-९\d]|अध्याय\s*विषय|पृष्ठ-संख्या|^\* [^*]+ \*$|संक्षिप्त|"
    r"मार्कण्डेयपुराण\s*समाप्त|श्रीमार्कण्डेयपुराण\s*सम्पूर्ण|"
    r"चौथा\s*अध्याय|पाँचवाँ|छठा|सप्तम|अष्टम|नवम|दशम|देवीमाहात्म्य|"
    r"अध्याय\s*पूरा\s*हुआ|उपसंहार\s*और\s*माहात्म्य",
    re.I,
)
GARBAGE = re.compile(
    r"(?:खरीदें|गीताप्रेस|मुद्रण|ISBN|file:|कुल मुद्रण|भूमिका|Digitzed|"
    r"Public Domain|गीताप्रेस, गोरख्पुर|फोन\s*:|फैक्स)"
)
...
def main() -> None:
    ...
    print(
        f"  markandeypurana: {len(main_blocks):5} blocks "
        f"(json ch4–90: {bulk_verses:5} verses)"
    )

    payload = {
        "source": f"https://archive.org/details/{IDENT}",
        "license": (
            "Gita Press Gorakhpur Hindi Markandeya Purana "
