"""Extract Hindi for Vayu Purana from archive.org Sanskrit-Hindi OCR.

Source: archive.org wg1194
  WG1194-1987 -Vayu Puran_djvu.txt

Continuous Hindi stream from chapter 4 onward (GRETIL rkv ch 4–232).
Mapped in seed-vayupurana-hindi.ts.

Output: scripts/cache/vayupurana-hindi.json

Run: python scripts/extract-vayupurana-hindi.py
"""
...
IDENT = "wg1194"
DJVU_FNAME = "WG1194-1987 -Vayu Puran_djvu.txt"
CACHE_PATH = ROOT / "scripts/cache/vayupurana-hindi-djvu.txt"
JSON_PATH = ROOT / "public/data/scriptures-full/vayupuran.json"
OUT_PATH = ROOT / "scripts/cache/vayupurana-hindi.json"

BULK_START_LINE = 1961

DEV = re.compile(r"[\u0900-\u097f]")
HINDI_MARKERS = re.compile(
    r"(?:है|होता|होते|कर|से|के|की|का|को|में|इस|यह|वह|चाहिए|चाहिये|कहा|जाता|"
    r"नहीं|करना|हुआ|हुए|अर्थात्|क्योंकि|इसलिये|भगवान|विष्णु|पुराण|वायु|"
    r"धर्म|मुनि|देव|राजा|ब्रह्म|श्रवण|पाप|मोक्ष|ऋषि|यज्ञ|लोक|सूत|शिव)"
)
...
PAGE_HEADER = re.compile(
    r"^\*?\s*वायुपुराण|वायुमहापुराण|^\d{1,4}\s+\*?$|^\s*॥ [^०-९\d]|अध्याय\s*विषय|"
    r"पृष्ठ-संख्या|^\* [^*]+ \*$|संक्षिप्त|अनुक्रमणिका|उपसंहारपादः समाप्त|"
    r"श्री गुरुचरणाभ्यां|शिवमस्तु",
    re.I,
)
GARBAGE = re.compile(
    r"(?:खरीदें|गीताप्रेस|मुद्रण|ISBN|file:|कुल मुद्रण|भूमिका|Digitzed|"
    r"एपपएप|फफखफ|पुस्तके तद्यथा)"
)
...
def main() -> None:
    ...
    print(f"  main: {len(main_blocks):5} blocks (json ch4–232: {bulk_verses:5} verses)")

    payload = {
        "source": f"https://archive.org/details/{IDENT}",
        "license": (
            "1987 Vayu Purana Sanskrit-Hindi edition (public domain scan, archive.org OCR)."
        ),
        "seedChapterStart": 4,
