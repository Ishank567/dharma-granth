"""Extract Hindi for Skanda Purana Revakhanda from Gita Press condensed OCR.

Source: archive.org 18mahapuran
  Skanda Puran_djvu.txt

रेवाखण्ड only (GRETIL rks, JSON chapters 5–116). आवन्त्यखण्ड section
lines 84105–92192 — bulk from Revakhanda adhyāya 5 (Purukutsa avatāra).
Chapters 1–4 keep curated highlights.

Output: scripts/cache/skandapurana-hindi.json

Run: python scripts/extract-skandapurana-hindi.py
"""
...
IDENT = "18mahapuran"
DJVU_FNAME = "Skanda Puran_djvu.txt"
CACHE_PATH = ROOT / "scripts/cache/skandapurana-hindi-djvu.txt"
JSON_PATH = ROOT / "public/data/scriptures-full/skandapuran.json"
OUT_PATH = ROOT / "scripts/cache/skandapurana-hindi.json"

BULK_START_LINE = 84105
BULK_END_LINE = 92192

DEV = re.compile(r"[\u0900-\u097f]")
HINDI_MARKERS = re.compile(
    r"(?:है|होता|होते|कर|से|के|की|का|को|में|इस|यह|वह|चाहिए|चाहिये|कहा|जाता|"
    r"नहीं|करना|हुआ|हुए|अर्थात्|क्योंकि|इसलिये|भगवान|विष्णु|पुराण|स्कन्द|शिव|"
    r"धर्म|मुनि|देव|राजा|ब्रह्म|श्रवण|पाप|मोक्ष|ऋषि|यज्ञ|लोक|कार्तिकेय|नर्मदा|रेवा)"
)
...
PAGE_HEADER = re.compile(
    r"^\*?\s*स्कन्दपुराण|^\d{1,4}\s+\*?$|^\s*॥ [^०-९\d]|अध्याय\s*विषय|पृष्ठ-संख्या|"
    r"^\* [^*]+ \*$|संक्षिप्त|स्कन्दपुराण\s*सम्पूर्ण|स्कन्दपुराण\s*समाप्त|"
    r"आवन्त्यखण्ड-रेवाखण्ड|रेवा-खण्ड सम्पूर्ण|आवन्त्यखण्ड समाप्त",
    re.I,
)
GARBAGE = re.compile(
    r"(?:खरीदें|गीताप्रेस|मुद्रण|ISBN|file:|कुल मुद्रण|भूमिका|Digitzed|"
    r"Skandpuran_Section|Public Domain)"
)
...
def main() -> None:
    ...
    print(
        f"  revakhanda: {len(main_blocks):5} blocks "
        f"(json ch5–116: {bulk_verses:5} verses)"
    )

    payload = {
        "source": f"https://archive.org/details/{IDENT}",
        "license": (
            "Gita Press Gorakhpur condensed Hindi Skanda Purana Revakhanda "
