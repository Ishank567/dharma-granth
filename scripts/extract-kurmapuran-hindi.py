"""Extract Hindi for Kurma Purana from Gita Press OCR.

Source: archive.org 18mahapuran
  Kurma Puran - Gita Press_djvu.txt

Continuous Hindi from पूर्वविभाग adhyāya 4 through उपरिविभाग ch 44.
Chapters 1–3 keep curated highlights.

Output: scripts/cache/kurmapuran-hindi.json

Run: python scripts/extract-kurmapuran-hindi.py
"""
...
IDENT = "18mahapuran"
DJVU_FNAME = "Kurma Puran - Gita Press_djvu.txt"
CACHE_PATH = ROOT / "scripts/cache/kurmapuran-hindi-djvu.txt"
JSON_PATH = ROOT / "public/data/scriptures-full/kurmapuran.json"
OUT_PATH = ROOT / "scripts/cache/kurmapuran-hindi.json"

BULK_START_LINE = 2987
BULK_END_LINE = 45936

DEV = re.compile(r"[\u0900-\u097f]")
HINDI_MARKERS = re.compile(
    r"(?:है|होता|होते|कर|से|के|की|का|को|में|इस|यह|वह|चाहिए|चाहिये|कहा|जाता|"
    r"नहीं|करना|हुआ|हुए|अर्थात्|क्योंकि|इसलिये|भगवान|विष्णु|पुराण|कूर्म|शिव|"
...
PAGE_HEADER = re.compile(
    r"^\*?\s*कूर्मपुराण|^\d{1,4}\s+\*?$|^\s*॥ [^०-९\d]|अध्याय\s*विषय|पृष्ठ-संख्या|"
    r"^\* [^*]+ \*$|संक्षिप्त|उपरिविभाग\s*समाप्त|कूर्मपुराण\s*समाप्त|"
    r"उपरिविभागः\s*समाप्त|पूर्वविभाग|उपरिविभाग|चौथा\s*अध्याय|पाँचवाँ|छठा|सप्तम|"
    r"अष्टम|नवम|दशम|ईश्वरगीता",
    re.I,
)
GARBAGE = re.compile(
    r"(?:खरीदें|गीताप्रेस|मुद्रण|ISBN|file:|कुल मुद्रण|भूमिका|Digitzed|"
    r"Public Domain|गीताप्रेस, गोरख्पुर)"
)
...
def main() -> None:
    ...
    print(
        f"  kurmapurana: {len(main_blocks):5} blocks "
        f"(json ch4–95: {bulk_verses:5} verses)"
    )

    payload = {
        "source": f"https://archive.org/details/{IDENT}",
        "license": (
            "Gita Press Gorakhpur Hindi Kurma Purana "
