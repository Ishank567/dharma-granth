/**
 * Seed English translations for Vayu Purana from G.V. Tagare's translation
 * (Motilal Banarsidass, UNESCO Indian Series).
 *
 * Chapters 1–3 keep curated highlights unless missing or `--force`.
 *
 * Prerequisite: python scripts/extract-vayupuran-tagare.py
 * Run: npm run seed:vayupuran-translations
 */
...
const PUB_PATH = resolve(ROOT, "public/data/scriptures-full/vayupuran.json");
const CACHE_PATH = resolve(ROOT, "scripts/cache/vayupuran-translations.json");
const SOURCE =
  "https://archive.org/details/the-vayu-purana-part-i-ancient-indian-tradition-and-mythology; https://archive.org/details/the-vayu-purana-part-2-ancient-indian-tradition-and-mythology";
...
async function main(): Promise<void> {
  log(`Seeding Vayu Purana from Tagare OCR cache${FORCE ? " [force]" : ""}…`);
  ...
  scripture.source = {
    ...
    translationLicense:
      "G.V. Tagare translation (Motilal Banarsidass, UNESCO Indian Series) — 111 adhyāyas mapped to GRETIL rkv 232 with global-stream fallback.",
