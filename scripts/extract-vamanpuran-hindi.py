"""Extract Hindi for Vamana Purana from Gita Press OCR.

Source: archive.org 18mahapuran
  Vaman Puran_djvu.txt

Continuous Hindi from adhyāya 4 through chapter 95.
Chapters 1–3 keep curated highlights.

Output: scripts/cache/vamanpuran-hindi.json

Run: python scripts/extract-vamanpuran-hindi.py
"""
...
IDENT = "18mahapuran"
DJVU_FNAME = "Vaman Puran_djvu.txt"
CACHE_PATH = ROOT / "scripts/cache/vamanpuran-hindi-djvu.txt"
JSON_PATH = ROOT / "public/data/scriptures-full/vamanpuran.json"
OUT_PATH = ROOT / "scripts/cache/vamanpuran-hindi.json"

BULK_START_LINE = 1689
BULK_END_LINE = 45597

DEV = re.compile(r"[\u0900-\u097f]")
HINDI_MARKERS = re.compile(
    r"(?:है|होता|होते|कर|से|के|की|का|को|में|इस|यह|वह|चाहिए|चाहिये|कहा|जाता|"
    r"नहीं|करना|हुआ|हुए|अर्थात्|क्योंकि|इसलिये|भगवान|विष्णु|पुराण|वामन|शिव|"
    r"धर्म|मुनि|देव|राजा|ब्रह्म|श्रवण|पाप|मोक्ष|ऋषि|यज्ञ|लोक|नारायण|इन्द्र|"
    r"पुलस्त्य|नारद|कपाली)"
)
...
PAGE_HEADER = re.compile(
    r"^\*?\s*श्रीवामनपुराण|^\d{1,4}\s+\*?$|^\s*॥ [^०-९\d]|अध्याय\s*विषय|पृष्ठ-संख्या|"
    r"^\* [^*]+ \*$|संक्षिप्त|वामनपुराण\s*समाप्त|श्रीवामनपुराण\s*समाप्त|"
    r"चौथा\s*अध्याय|पाँचवाँ|छठा|सप्तम|अष्टम|नवम|दशम|सरोमाहात्म्य",
    re.I,
)
GARBAGE = re.compile(
...
def download_djvu() -> str:
    if CACHE_PATH.exists():
        ...
        if sum(1 for char in text if "\u0900" <= char <= "\u097f") > 400000:
            return text
...
def main() -> None:
    ...
    print(
        f"  vamanpurana: {len(main_blocks):5} blocks "
    ...
    payload = {
        ...
        "license": (
            "Gita Press Gorakhpur Hindi Vamana Purana "
