"""Extract Hindi for Agni Purana from Gita Press condensed edition OCR.

Source: archive.org 18mahapuran
  Agni Puran - Gita Press_djvu.txt

Single continuous संक्षिप्त अग्निपुराण Hindi stream from chapter 4 onward.
Mapped in seed-agnipurana-hindi.ts:
  main → GRETIL chapters 4–381

Output: scripts/cache/agnipurana-hindi.json

Run: python scripts/extract-agnipurana-hindi.py
"""
...
IDENT = "18mahapuran"
DJVU_FNAME = "Agni Puran - Gita Press_djvu.txt"
CACHE_PATH = ROOT / "scripts/cache/agnipurana-hindi-djvu.txt"
JSON_PATH = ROOT / "public/data/scriptures-full/agnipuran.json"
OUT_PATH = ROOT / "scripts/cache/agnipurana-hindi.json"

BULK_START_LINE = 1627

DEV = re.compile(r"[\u0900-\u097f]")
HINDI_MARKERS = re.compile(
    r"(?:है|होता|होते|कर|से|के|की|का|को|में|इस|यह|वह|चाहिए|चाहिये|कहा|जाता|"
    r"नहीं|करना|हुआ|हुए|अर्थात्|क्योंकि|इसलिये|भगवान|विष्णु|पुराण|अग्नि|"
    r"धर्म|मुनि|देव|राजा|ब्रह्म|श्रवण|पाप|मोक्ष|ऋषि|यज्ञ|लोक|व्यास|वसिष्ठ)"
)
...
PAGE_HEADER = re.compile(
    r"^\*?\s*अग्निपुराण|^\d{1,4}\s+\*?$|^\s*॥ [^०-९\d]|अध्याय\s*विषय|पृष्ठ-संख्या|"
    r"^\* [^*]+ \*$|संक्षिप्त परिचय|अग्निपुराण\s*सम्पूर्ण",
    re.I,
)
GARBAGE = re.compile(
    r"(?:खरीदें|गीताप्रेस|मुद्रण|ISBN|file:|कुल मुद्रण|भूमिका|Digitzed|"
    r"फफफ|कफ\s*फ|EEE|अप्ज्ज)"
)
...
def main() -> None:
    ...
    print(f"  main: {len(main_blocks):5} blocks (json ch4–381: {bulk_verses:5} verses)")

    payload = {
        "source": f"https://archive.org/details/{IDENT}",
        "license": (
            "Gita Press Gorakhpur condensed Hindi Agni Purana "
