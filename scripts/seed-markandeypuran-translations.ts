/**
 * Seed English translations for Markandeya Purana (adhyāyas 4–90) from
 * F. Eden Pargiter's translation (1904, public domain).
 *
 * Chapters 1–3 keep curated highlights (Devi Mahatmyam) unless missing or `--force`.
 *
 * Prerequisite: python scripts/extract-markandeya-pargiter.py
 * Run: npx tsx scripts/seed-markandeypuran-translations.ts
 */
...
const CACHE_PATH = resolve(ROOT, "scripts/cache/markandeypuran-translations.json");
const PARGITER_SOURCE =
...
async function main(): Promise<void> {
  log(`Seeding Markandeya Purana from Pargiter OCR cache${FORCE ? " [force]" : ""}…`);
  ...
  scripture.source = {
    ...
    translationLicense:
      "F. Eden Pargiter translation (1904), Asiatic Society of Bengal — public domain.",
