/**
 * Seed Hindi for Atharva Veda kandas 4–20 from Vidyalankar bhashya cache.
 *
 * Kandas 1–3 keep curated highlights unless `--force`.
 *
 * Prerequisite: python scripts/extract-atharvaveda-hindi.py
 * Run: npm run seed:atharvaveda-hindi
 */
...
const CACHE_PATH = resolve(ROOT, "scripts/cache/atharvaveda-hindi.json");
...
interface HindiCache {
  source: string;
  license: string;
  kandas: Record<string, string[]>;
}

async function main(): Promise<void> {
  log(`Seeding Atharva Veda Hindi from Vidyalankar bhashya cache${FORCE ? " [force]" : ""}…`);
  ...
  for (const chapter of scripture.chapters) {
    ...
    const blocks = cache.kandas[String(chapter.number)] ?? [];
