"""Extract Hindi commentary blocks for Chandogya Upanishad from Gita Press DJVU OCR.

Source: archive.org phAO_chandogya-upanishad-gita-press-gorakhpur
  Chandogya Upanishad - Gita Press Gorakhpur_djvu.txt

The edition is Shankara bhashya with Hindi tika — not verse-aligned, so blocks are
mapped sequentially to published verses in seed-chandogya-hindi.ts.

Output: scripts/cache/chandogya-hindi.json

Run: python scripts/extract-chandogya-hindi.py
"""
...
IDENT = "phAO_chandogya-upanishad-gita-press-gorakhpur"
DJVU_PATH = ROOT / "scripts/cache/chandogya-gita-press-djvu.txt"
JSON_PATH = ROOT / "public/data/scriptures-full/chandogya.json"
OUT_PATH = ROOT / "scripts/cache/chandogya-hindi.json"
...
NOISE = re.compile(
    r"^(?:अन्वय|पदार्थ|मूलम्|इति|श्लोक|खण्ड|मन्त्र|अध्याय|प्रपाठक|सम्पादक|"
    r"शङ्करभाष्य|शांकरभाष्य|^\d+\s*$|file:)",
    re.I,
)
...
def extract_blocks(text: str) -> list[str]:
    start = text.find("ओमित्येतदक्षरम्")
    if start < 0:
        start = text.find("छान्दोग्योपनिषद्")
    if start < 0:
        start = 0
    body = text[start:]
