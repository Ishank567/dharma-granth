...
export default function BookCard({ book, categorySlug }: { book: Book; categorySlug: string }) {
  return (
    ...
    >
      ...
      <div className="mb-4 flex h-20 items-center justify-center rounded-xl bg-gradient-to-br from-accent-bg to-card text-3xl">
        📜
      </div>

      <h3 className="font-serif-deva text-lg font-bold text-foreground group-hover:text-primary transition-colors leading-tight">
        {book.title_hindi}
      </h3>
      ...
      <div className="mt-3 flex flex-wrap gap-2">
        ...
        {book.total_pages > 0 && (
          <span className="inline-flex items-center rounded-full bg-card-hover px-2.5 py-0.5 text-xs text-muted">
            {book.total_pages} पृष्ठ
          </span>
        )}
      </div>
    </Link>
